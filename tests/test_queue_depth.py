...
def test_step_queue_depth_reorders_queue_heads(monkeypatch, capsys):
    ...
    assert "queue-head mode: H rows sorted by descending depth" in output
    assert "H     28" in output
...
def test_step_default_is_queue_head_value_order(monkeypatch, capsys):
    monkeypatch.setattr(cli_module, "_load_ew_terms", lambda count: tuple(EW_PREFIX[:count]))

    cli_module.step(11, max_width=0)

    output = capsys.readouterr().out
    assert "queue-head mode: one current unused head" in output
